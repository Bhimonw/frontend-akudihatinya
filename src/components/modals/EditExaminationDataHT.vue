...
export default {
  ...
  data() {
    return {
      ...
      errors: {},
      flatpickrConfig: {
        dateFormat: "Y-m-d", // Format Y-m-d untuk kompatibilitas dengan backend & <input type="date">
    ...
    examData: {
      ...
      handler(newData) {
        if (newData && newData.id) {
          this.formData = {
            date: newData.examination_date || null,
            systolic: newData.systolic || null,
            diastolic: newData.diastolic || null,
          };
          this.errors = {}; // Reset error saat data baru masuk
        }
      },
    },
    visible(newValue) {
  ...
  methods: {
    validateForm() {
      ...
      if (hasError) {
        ...
        if (firstErrorKey) {
          const inputRef = this.$refs[`${firstErrorKey}Input`];
    ...
    validateSystolic() {
      ...
      if (val === null || val === undefined || val === '') {
        this.errors.systolic = "Nilai Sistolik wajib diisi.";
        return;
      }
      if (val < 50 || val > 300) {
    ...
    validateDiastolic() {
      ...
      if (val === null || val === undefined || val === '') {
        this.errors.diastolic = "Nilai Diastolik wajib diisi.";
        return;
      }
      if (val < 30 || val > 200) {
    ...
    async handleSubmit() {
      try {
        ...
        console.log("Payload being sent for HT update:", payload);
        
        await axios.put(
          `http://localhost:8000/api/puskesmas/ht-examinations/${this.examData.id}`,
          payload,
          {
            headers: {
